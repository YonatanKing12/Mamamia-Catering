 ...
 * שאפשר לבחור בהם: «תפריט שף» בסקשן ‎02, הבחירה שמזריעה `service_format`
 * לבנאי ההצעה, והחומר של עמודי האירועים.
 *
 * גם הוא ריק, ומאותה סיבה: תפריט הוא הצהרה של הלקוח על מה שהמטבחים
 * מוציאים בפועל ובאיזו צורת הגשה. שם תפריט שהומצא כאן הוא מוצר שהעסק
 * לא מוכר.
 *
 * ─────────────────────────────────────────────────────────────────────
 *  החוק
 ...
 *   5. **לאילו סוגי אירוע הוא מתאים** — אופציונלי, מתוך `OCCASIONS`.
 *      שיוך כאן אינו הצהרה שהעמוד קיים או שהאירוע נתמך; הוא רק אומר
 *      לעמוד שכן קיים איזה תפריט להראות.
 ...
 *   kickerHe: "מה שיוצא מהמטבח בהרצליה בערב רגיל",
 ...
 * ```
 *
 * ‎spec: 01 §6.2, 02 §3.4 (סקשן ‎02 מרונדר רק לצורת הגשה שאושרה),
 * 02 §4.1 (בחירת מנה מזריעה את הבנאי), 02 §2.2 (מנעולי המחיר).
 */
...
/**
 * סוגי האירוע שיש להם עמוד ייעודי במפרט (01 §4 P-08…P-14). המזהים זהים
 * למקטע האחרון של הנתיב, כדי שקישור עמוק ייבנה בלי טבלת תרגום.
 *
 * זו אוצר מילים לתיוג ולניתוב בלבד — **לא רשימת שירותים**. אין לרנדר
 * ממנו רשימה של «אירועים שאנחנו עושים»: עמוד שלא נבנה אינו מקבל קישור,
 * ואירוע שהלקוח לא אישר אינו הופך למוצר בגלל שהוא מופיע כאן. מחרוזות
 * הנתיב עצמן שייכות למודול הניתוב, לא לכאן.
 */
...
/**
 * התפריטים שהמטבח הזה יכול להוציא — כלומר שיש בהם מנה שהוא מבשל.
 * הסינון של עמוד סניף.
 */
