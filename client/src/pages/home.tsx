 ...
 *
 * מה שנשאר הוא מה שאנחנו באמת יודעים: שלושה מטבחים, איך התהליך עובד,
...
const MenuHero = () => {
  ...
  /* שורת העובדות — גיזום ברמת הפסוקית (G5). כל אסימון ריק מוריד את
     ...
     `שלושה מטבחים` — היחידה שהיא עובדה שבידינו. */
    ...
    <section className="pb-sec pt-[clamp(2.5rem,7vw,5rem)]">
      <div className="wrap">
        ...
        <Prose size="lede" measure="lede" className="mt-7">
          <p>
            מאמא מיה היא מסעדה איטלקית עם שלושה מטבחים פעילים:{" "}
            {branchSentence(names)}. מה שכתוב כאן מתבשל במטבחי המסעדה.
          </p>
        </Prose>
    ...
    <section id="kitchens" className="sec sec--alt">
      <div className="wrap">
        <SectionHeader
          num={num}
          eyebrow="המטבחים"
          title="שלושה מטבחים פעילים, לא מטבח ייצור"
          /* בלי `כל יום` ובלי `הערב`: שעות הפעילות הן Slot ריק, וכל אחת
             מהמילים האלה היא טענה עליהן. */
          lede="מסעדה איטלקית עם שלושה מטבחים שמבשלים לסועדים במסעדה. הקייטרינג יוצא מאותם מטבחים, ובידיים של אותם טבחים."
        />
