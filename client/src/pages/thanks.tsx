 ...
 * ─────────────────────────────────────────────────────────────────────
 * כאן, ורק כאן, הסדר מתהפך — ובכוונה: **הבנייה כבר קרתה.** הצעת פקד
 * «בנו תפריט» לאדם ששלח בקשה לפני שתי שניות אומרת לו שהבקשה לא נקלטה.
 * לכן:
 *   1. **וואטסאפ — הפקד הממולא היחיד בעמוד,** עם מספר הפנייה בתוך
 *      ההודעה, כדי שהשיחה תיתפר לאותה שורת ליד ולא תיצור שנייה.
 *   2. **טלפון — קישור טקסט.**
 *   3. **בנייה — אינה מוצעת כאן בכלל.** הפס הדביק כבוי (`stickyBar:'none'`,
 *      02 §1.2) בדיוק מאותה סיבה.
 *
 * ─────────────────────────────────────────────────────────────────────
 ...
 *                 `server/routes.ts`. עד שיהיה, אין מה להדהד מעבר
 *                 ל־`history.state` שכבר קיים כאן.
 ...
 *   קישור ל־`/summary`
 *                 המסלול קיים ומופעל, אבל **הקישור שנשלח הלאה נשבר**:
 *                 ‎`/summary` נשען על `GET /api/quote/:ref`, ובלעדיו כל
 *                 נמען שאינו הלשונית הזאת מקבל «הסיכום כבר לא זמין».
 *                 להציע לקונה מסמך להעברה ולתת לו קישור שנשבר אצל
 *                 המאשר גרוע מלא להציע כלום. זה המנגנון בעל התשואה
 *                 הגבוהה ביותר שחסום היום, והוא מדווח ככזה.
 ...
 *   זמן תגובה     ‎`SLOTS.responseTime` ריק. «נחזור אליכם בהקדם» הוא
 *                 התחייבות שאיש לא אישר, ולכן המשפט מתקצר ואינו מנחש.
 *
 * ─────────────────────────────────────────────────────────────────────
      ...
      <section className="pb-sec pt-[clamp(2.5rem,7vw,4.5rem)]">
        <div className="wrap">
          <div className="max-w-confirm">
            ...
            {/* מספר הפנייה הוא החפץ היחיד שהקונה לוקח איתו מהעמוד: הוא
                מוקרא בטלפון, נשלח בהודעה ומודבק בקבוצה. לכן הוא כרטיס
                מוגבה עם תווית ענבר, ולא שורה בתוך פסקה — וספרות טבלאיות,
                מבודדות דו־כיוונית וניתנות לסימון בקליק אחד. */}
            {ref ? (
              <div className="mt-8 rounded-card border border-solid border-[color:var(--rule)] bg-bg-form p-card">
                {/* לא `.eyebrow`: המחלקה ההיא קובעת `color:var(--fg-subtle)`
                    בתוך שכבת ה־utilities, ו־`text-accent` באותה שכבה אינו
                    מובטח לגבור עליה. אותם ערכים בדיוק, בענבר. */}
            ...
            <Prose measure="confirm" className="mt-8">
              <p>
                ...
                {ref
                  ? "שמרו את המספר. הוא מקצר כל שיחה איתנו."
                  : "אם קיבלתם מספר פנייה, שמרו אותו. הוא מקצר כל שיחה איתנו."}
              </p>
              {routedBranch ? <p>הפנייה נשלחה למטבח ב{routedBranch}.</p> : null}
              <p>רוצים להתקדם עכשיו? בוואטסאפ ההודעה נפתחת עם המספר כבר בפנים.</p>
            </Prose>
            ...
            {/* הפקד הממולא היחיד בעמוד. `data-print="hide"` — דף אישור
