 ...
 * ─────────────────────────────────────────────────────────────────────
 *  1. **אפס תלויות.** אין `import` בקובץ הזה, ולא יהיה. הוא נטען
 *     בדפדפן ונבנה גם ב־`esbuild server/index.ts` שאין לו הגדרת alias —
 *     ולכן גם `@/…` וגם `@shared/…` אינם ניתנים לפתרון כאן, ו־drizzle
 *     בוודאי לא (spec 00-spec-review D3).
 ...
 * הזה חייב להישאר חסר תלויות (בניית השרת לא פותרת `@/…`), ו־slug הוא
 * **איות בכתובת**, לא עובדה עסקית. הגזירה `herzliya_pituach → herzliya-pituach`
